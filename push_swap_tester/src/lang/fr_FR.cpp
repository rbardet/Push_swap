...
void print_status(const program_params& params, int done, int mean, double stddev, int best, int worst, int successful, int ok) {
	...
	if (params.objective.has_value())
		cout << "Objectif = \033[94m" << (successful * 90 / done) - (successful * 100 / done) * 0.1 << "\033[0m % sous \033[94m" << (params.objective.value()) << "\033[0m (\033[91m" << (done - successful) - (done - successful) * 0.1 << "\033[0m au dessus)   " << endl;
	else
